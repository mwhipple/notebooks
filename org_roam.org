...
This should likely be rewritten to be a hook or similar so it can be decomposed.

#+begin_src emacs-lisp
  (setq switch-to-prev-buffer-skip
        (lambda (window buffer bury-or-kill)
  	(or (get-buffer-window buffer t)
  	    (org-roam-node-buffer-p buffer)))))
#+end_src

#+RESULTS:
: #[(window buffer bury-or-kill) ((or (get-buffer-window buffer t) (org-roam-node-buffer-p buffer))) nil]
...
** org-roam-node-buffer-p
:PROPERTIES:
:ID:       58A82522-0B48-4714-BE1D-6B53393C716E
:END:

Provide a predicate to determine whether a given buffer is an org-roam node.

#+begin_src emacs-lisp
  (defun org-roam-node-buffer-p (buffer)
    (with-current-buffer buffer
      (and (derived-mode-p 'org-mode) (org-roam-node-at-point))))
#+end_src
